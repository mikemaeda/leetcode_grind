...
export default async function Home() {
  ...
  const submissionRows = await db.select({ id: problemSubmissions.id, memberId: users.id, memberName: users.name, problemTitle: problemSubmissions.problemTitle, leetcodeUrl: problemSubmissions.leetcodeUrl, submittedAt: problemSubmissions.submittedAt, proofUrl: proofUploads.imageUrl, verificationStatus: proofUploads.verificationStatus }).from(problemSubmissions).innerJoin(users, eq(users.id, problemSubmissions.userId)).innerJoin(dailyCommitments, eq(dailyCommitments.id, problemSubmissions.commitmentId)).innerJoin(proofUploads, eq(proofUploads.submissionId, problemSubmissions.id)).where(and(eq(dailyCommitments.groupId, LEETCODE_GROUP_ID), eq(dailyCommitments.date, date))).orderBy(problemSubmissions.submittedAt);
  const submissions = Array.from(submissionRows.reduce((map, row) => { const existing = map.get(row.id); if (existing) existing.proofUrls.push(row.proofUrl); else map.set(row.id, { id: row.id, memberId: row.memberId, memberName: row.memberName, problemTitle: row.problemTitle, leetcodeUrl: row.leetcodeUrl, submittedAt: row.submittedAt, proofUrls: [row.proofUrl], verificationStatus: row.verificationStatus }); return map; }, new Map<string, { id: string; memberId: string; memberName: string; problemTitle: string; leetcodeUrl: string; submittedAt: string; proofUrls: string[]; verificationStatus: string }>()).values());
